...
bitext_rsh::~bitext_rsh() {				// inserted this line and deleted
}			  // the deconstructor that uses nullptr not supported by c++98
...
void bitext_rsh::compute_r_l() {
	l = ceil(log2((long double)pp.n) + 2*log2(2.0/pp.eps));
	r = ceil((1.0*pp.n)/(1.0*l));  			// chg:  r = ceil(pp.n/l);
printf("l=%lu, r=%lu, n=%lu, log(n)=%Lf, log(2/err)=%f, log(1/err)=%f\n", l, r, pp.n, log2((long double)pp.n), log2(2.0/pp.eps), log2(1.0/pp.eps)  );
	
	uint64_t l_rounded = l + (8-l%8); // See comment in num_random_bits
	chars_per_half = l_rounded/(8*sizeof(char));	// replaced undefined BITS_PER_TYPE(char) with 8*sizeof(char)
...
//		vec_wrd =  pad/sizeof(chunk_t); // start wrd loc of MSB
                if (pad == 0) {   // output bit loc within Byte the same for SSL & NTL, except when pad NOT 0
                        vec_wrd = 0;
                } else {
                        vec_wrd =  (byte_len-1)-len/sizeof(chunk_t); // start wrd loc of MSB
                }
	#endif
