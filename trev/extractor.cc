...
#include <pthread.h>
//#include "ossl_locking.h"					// not using locking or pthreads.h
#include "utils.hpp"
...
pthread_cond_t  cond_wait_on_go    = PTHREAD_COND_INITIALIZER;
pthread_cond_t  cond_wait_on_done  = PTHREAD_COND_INITIALIZER; 
pthread_cond_t  cond_wait_on_start = PTHREAD_COND_INITIALIZER; 
pthread_mutex_t go_lock 	   = PTHREAD_MUTEX_INITIALIZER; 
...
int thread_m_index  = 0;
int thread_go_cnt   = 0;
int thread_done_cnt = 0;

bitfield<unsigned int, uint64_t> *thread_init_rand_bf;
params thread_params;
bitext *thread_bext;
weakdes *thread_wd;
uint64_t thread_t;
int *thread_extracted_rand;
uint64_t thread_num_rand_bits;
...
void *alloc_and_zero(size_t m) { 
	int *mem = (int*)do_alloc((m/(8*sizeof(int))+1)*sizeof(int), "zero mem");  // replaced undefined BITS_PER_TYPE(int) with 8*sizeof(int)

	memset(mem, 0, (m/(8*sizeof(int))+1)*sizeof(int));	// clr full words` 
...
void Si_test (uint64_t rows, weakdes *wd, params &params) {	
        int *histo = (int*)alloc_and_zero( (32*params.pp.d) );  // histogram of seed usage over "d"
	int distro_limit = 8*1024;
        int *distr_histo = (int*)alloc_and_zero( (32*distro_limit) );   // overlap histogram distribition
...
//			printf("%ld ", indices[j] );
                        if (indices[j] < params.pp.d) {
                                ...
                                printf("***Error in compute_Si for %ld row and %d index, indices[j]=%ld \n", i, j, indices[j] );
                        }
                }
//			printf("\n\n" );
        }
...
//				printf(" histo[%d]=%d \n", j, histo[j] );
        ...
        printf("(i.e., expect to see 2 or 3 entries, [0], [n] & maybe [n+1], otherwise not uniformly accessed)\n");
...
void gen_blk_design(params &params) {
 ...
 cout << "ptr_base_mi->size()=" << ptr_base_mi->size() << endl;
	...
	ptr_mi->at(0) = ceil(ni);
	ptr_base_mi->at(0) = 0;
//	if (ptr_mi->at(0) < params.pp.t_pri) t_threshold = ptr_mi->at(0); else t_threshold = params.pp.t_pri; 
	if (ptr_mi->at(0) < params.pp.t_pri) { // special case, use t_pri as min for each full blk
		if ( params.pp.m_req > (params.pp.a+1)*params.pp.t_pri)  {
		      cout << "m_req>a*t_pri " << params.pp.m_req << " > " << params.pp.a*params.pp.t_pri << " special case prob in gen_blk_design, Terminating" << endl;
		      exit(-1);	
		}
		ptr_mi->at(0) = params.pp.t_pri;
		sum_ni        = params.pp.t_pri;
		int i = 1;
		while (sum_ni <= params.pp.m_req) {
			if ( (params.pp.m_req-sum_ni) > params.pp.t_pri) {
				ptr_mi->at(i) 	   = params.pp.t_pri;
				ptr_base_mi->at(i) = sum_ni;
				sum_ni            += params.pp.t_pri;
				i++;
			} else {
				ptr_mi->at(i)      = params.pp.m_req-sum_ni;
                                ptr_base_mi->at(i) = sum_ni;
				cout << "i new a=" << i << ", a=" << params.pp.a << endl;
				params.pp.a = i;
				return;
			}
		}
		cout << "Failed special case prob in gen_blk_design, Terminating" << endl;
		exit(-1);
	}
...
cout << i << ". " <<  ptr_mi->at(i) << "  " <<  ptr_base_mi->at(i) << endl;
	...
	ptr_base_mi->at(params.pp.a) = ptr_mi->at(params.pp.a-1) + ptr_base_mi->at(params.pp.a-1); 
	ptr_mi->at(params.pp.a) = params.pp.m_req - ptr_base_mi->at(params.pp.a);   // use total output size "pp.m_req"
 ...
 cout << params.pp.a << ". " <<  ptr_mi->at(params.pp.a) << "  " <<  ptr_base_mi->at(params.pp.a) << endl;
...
uint64_t rd_ascii_bit_file(ifstream & file_handle, uint64_t bits_needed, vector<uint64_t> *data_vector){
	char c;
	uint64_t cnt = 0;
	uint64_t num = 0;
	...
	if ( num > 0 )
		cout << "*** " << cnt << " 1's and 0's read along with " << num << " num of other chars" << endl;
	return (cnt);
}
...
// algorithms
//
	//removed variables relating to parallelism
	//and also passes a result-save option "out_data".

void* trevisan_extract_thread(void* thread_data) {
...
//	bitfield<unsigned int, uint64_t> &init_rand_bf;
	...
	uint64_t t;
	int *extracted_rand;
	uint64_t num_rand_bits;	
	uint64_t i;

	vector<unsigned int> y_S_i_list;					// use only for 1st half of sub-seed 
	vector<unsigned int> y_S_ib_list;					// need a 2nd list for the 2nd half of sub_seed
	vector<uint64_t> indices;
	unsigned int *y_S_i;
	unsigned int *y_S_ib;
	Thread_arg *my_thread_info;
	my_thread_info = (Thread_arg *)thread_data;
cout << "Starting Thread, ID = " << my_thread_info->my_id << endl;
THREAD_STREAM_LOOP:			
	   // wait for start signal
     	pthread_mutex_lock(&go_lock);
	while (thread_go_cnt == 0) { 	// go_cnt set to num_threads to indicate start
      		pthread_cond_wait(&cond_wait_on_go, &go_lock); // wait for start from trevisan_dispatch, go_cnt > 0
	}
	thread_go_cnt--;	// decr go_cnt when thread starts, when 0 then all threads have started
	if (thread_go_cnt == 0)
		 pthread_cond_signal(&cond_wait_on_start);	// wakeup any finished threads wait for all to start
      	else
		pthread_cond_signal(&cond_wait_on_go);
	pthread_mutex_unlock(&go_lock);
// cout << "Starting Thread, Init Wakeup, thread_go_cnt = " << thread_go_cnt << endl;
	   //  get params 
	// init_rand_bf = thread_init_rand_bf;
	bitfield<unsigned int, uint64_t> &init_rand_bf = *thread_init_rand_bf;
	params = thread_params;
	bext = thread_bext;
	wd = thread_wd;
	t = thread_t;
	extracted_rand = thread_extracted_rand;
	num_rand_bits = thread_num_rand_bits;	
	...
	y_S_i_list.resize(t);
	y_S_ib_list.resize(t);
	indices.resize(t);
	y_S_i   = &y_S_i_list[0];				
	y_S_ib  = &y_S_ib_list[0];				

     	pthread_mutex_lock(&index_lock);
		i = thread_m_index;	// get next i (output data) index
		thread_m_index++; 	// incr to next index
   ...
   while ( i < params.pp.m) {
	...
	wd->compute_Si(i, indices);	// in "weakdes_gfp.cc"
	...
	for ( int a=0; a<= params.pp.a; a++) { // repeat for each Blk use same seed loc but on a diff set of seed
		...
		// of randomness.
		memset(y_S_i, 0,  params.pp.t_req);  // Mink 9-10-2014 corrected to "t_req" size   // replaced undefined BITS_PER_BYTE with 8
		for (uint64_t count = 0; count < params.pp.l; count++) {		// chg for 1st half from:  count < t; count++) { 
			...
			if (indices[count] > num_rand_bits) {
				cerr << "Internal error: Bit index exceeds amount of available randomness (selected:" << indices[count]
				     << ", available: " << num_rand_bits << ", index: " << count << ")!" << endl;
				exit(-1);
			}
	#endif
			if (init_rand_bf.get_bit(indices[count]+cur_seed_base)) {	
				 y_S_i[count/(8*sizeof(unsigned int))] |=	// replaced undefined BITS_PER_TYPE(unsigned int) with 8*sizeof(unsigned int)
		...
		if (!params.skip_bitext) {
				 // added a 2nd vector & a parameter for output "out_data"
			if(bext->extract(y_S_i, y_S_ib)) {	 // added a 2nd vector & replaced "out_data"	  
				pthread_mutex_lock(&output_lock); 	// only lock output data access for a "1", already initialized to "0"
				extracted_rand[(i+ptr_base_mi->at(a))/(8*sizeof(int))] |=  // added "base_mi" offset to output & replaced undefined BITS_PER_BYTE with 8
...
// cout << "Completing Thread, Entering tests, thread_go_cnt = " << thread_go_cnt <<  endl;
 	pthread_mutex_lock(&go_lock);	// can't finish before everyone has started
        while (thread_go_cnt != 0) {
                pthread_cond_wait(&cond_wait_on_start, &go_lock); // wait for signal that go_cnt == 0
        }
        pthread_mutex_unlock(&go_lock);

// cout << "Completing Thread, Entering DONE tests " <<  endl;
 	pthread_mutex_lock(&done_lock);	// when all threads are done, alert trevisan_dispatch
	thread_done_cnt--;
        if (thread_done_cnt == 0) {
		pthread_cond_signal(&cond_wait_on_done);      // wakeup trevisan_dispatch to continue
        }
        pthread_mutex_unlock(&done_lock);

cout << "DONE  Thread, Entering DONE tests, thread_done_cnt = " << thread_done_cnt << endl;
	if (params.streaming)
		goto THREAD_STREAM_LOOP;
}
...
inline bool trevisan_extract(uint64_t i, vector<uint64_t> &indices,
			     bitfield<unsigned int, uint64_t> &init_rand_bf, unsigned int *y_S_i,
			     unsigned int *y_S_ib, params &params, bitext *bext, weakdes *wd,	// added a 2nd sub-seed vector
			     uint64_t t, int *extracted_rand, uint64_t num_rand_bits) { 			// replaced "ofstream & out_data"
...
void trevisan_dispatcher(class weakdes *wd, class bitext *bext, params &params) {
	...
	if (params.verbose) {
		...
		cout << static_cast<long double>(params.pp.m)/(params.pp.alpha*params.pp.n) << fixed << endl;		
		cout << "-------------------------------------------------------------"
		     << endl << endl;
	}
	...
	if (params.dryrun)
		...
		// detleted section about saving the weak design, currently unused.
...
double my_num = (log(1.0*params.pp.m_req-params.pp.r)-log(1.0*params.pp.t_pri-params.pp.r));
double my_den = ( log(params.pp.r)-log(params.pp.r-1.0));
double my_frac = my_num/my_den;
cout << "my_num, my_den, my_frac = " << my_num << ", " << my_den << ", " << my_frac << endl;
	uint64_t my_a = ceil( (log(1.0*params.pp.m_req-params.pp.r)-log(1.0*params.pp.t_pri-params.pp.r))/( log(params.pp.r)-log(params.pp.r-1.0)) ); 
	cout << "params.pp.a = " << my_a << endl;	
	if (params.blk_des && my_a<1) {				// prepare for Blk_design
		cout << "a=0, checking t, m = " << params.pp.t_req  << ", " << params.pp.m_req << endl;
		if ( params.pp.t_req < params.pp.m_req) {
			cout << "a=0, but t<m " << params.pp.t_req  << "<" << params.pp.m_req << ", prog treimating" << endl;
			exit(-1);
		}
		cout << "OK to fake a weak design with the number of requested output bit, since t>m there is no overlap" << endl;
		wd->set_params(t, params.pp.m_req);
		params.pp.m = params.pp.m_req;  // fake a weak design with the number of requested output bit, since there is no overlap
	}
	if (params.blk_des && my_a>0 ) {				// prepare for Blk_design
				// use "m_req" & init Blk cnt
		params.pp.a = ceil( (log(1.0*params.pp.m_req-params.pp.r)-log(1.0*params.pp.t_pri-params.pp.r))/( log(params.pp.r)-log(params.pp.r-1.0)) ); 
...
// cout << "----params.pp.base_mi[3]=" << params.pp.base_mi[3] << endl;
// cout << "****size params.pp.base_mi_array=" << params.pp.base_mi.size() << endl;
	...
	prng *initrandom = new prng;				// create new prng object in order to use prng.hpp's PRNG
	
		// Create the seed, initial randomness y (and make the result accessible as void pointer)
...
//		initrandom = &initial_rand_vector;
		initial_rand_vector.at(params.pp.d_tot/64) &= part_mask(params.pp.d_tot); // clr out unneeded lower bits when len is not aligned at 64
		init_rand_bf.set_raw_data( (void *) &initial_rand_vector, params.pp.d_tot); // chg for Blk_Des
  cout << "QRNG Streaming Seed Done" << endl; 
	 } else if (params.pp.q == "NA"){			//if no user-given source bits file, get bits with create_randomness(...)
		...
		if(data_file.is_open()) {
			...
			uint64_t len = rd_ascii_bit_file(data_file, params.pp.d_tot, &initial_rand_vector); // replace code with a better fct
			if ( len < params.pp.d_tot ) {
				cout << "Insufficient input data in Seed file, only " << len << " instead of specified " << params.pp.d_tot << endl;
				exit(-1);
			}
			data_file.close();
		} else {
			cout << "****Can't open specified Seed file " << params.pp.q << ", Terminating Program" << endl;
			exit(-1);
		}
		init_rand_bf.set_raw_data( (void *) &initial_rand_vector, params.pp.d_tot); // chg for Blk_Des
	}		
	...
	measure(&start);
 	...
 	if (params.num_threads > 0) {
 		thread_init_rand_bf = &init_rand_bf;  // init params for Threads
        	thread_params = params;
        	thread_bext = bext;
        	thread_wd = wd;
        	thread_t = t;
        	thread_extracted_rand = extracted_rand;
        	thread_num_rand_bits = num_rand_bits;

 		pthread_mutex_lock(&go_lock);	// init go_cnt & done_cnt & then signal all Threads to start
		thread_go_cnt   = params.num_threads;
		thread_done_cnt = params.num_threads;
                pthread_cond_signal(&cond_wait_on_go);      // wakeup all Threads to start
        	pthread_mutex_unlock(&go_lock);

        	pthread_mutex_lock(&done_lock); // wait until all threads are done
		while (thread_done_cnt > 0)
			pthread_cond_wait(&cond_wait_on_done, &done_lock);
        	pthread_mutex_unlock(&done_lock);
		...
		for (uint64_t i = 0; i <params.pp.m; i++) {	// loop thru each (row) extracted bit, for Blk_des each Blk is proc by at the same time
			trevisan_extract(i, indices, init_rand_bf, &y_S_i[0],  &y_S_ib[0],	// added 2nd sub-seed list
	...
	cout << "Required " << dec << delta_to_ms(delta) << " ms for "
	     << params.pp.m_req << " bits extracted from " << params.pp.n << " bits, using " 
	     << params.pp.d_tot << " Seed bits. "<< endl;
...
void parse_cmdline(struct params &params, int argc, char **argv) {
	try {
		CmdLine cmd("Trevisan extractor framework", ' ', "Sep 2012");
		 ...
		 ValueArg<string> loadArg("i", "input",
					     "Enter your file containing bit input",	// added this to allow user-input source bits
					     false, "NA", "string");
		ValueArg<string> seedArg("q", "seed",
					     "Enter your file containing bit seed",	// added this to allow user-input seed bits
					     false, "NA", "string");
		ValueArg<string> outfArg("o", "output_file",
					     "Enter your output extracted_data file name", // added this to allow user-output data file for extracted data
					     false, "NA", "string");
		SwitchArg blockArg("b", "Blk_Design", "Enable Block Design", false);	// added this to allow user to select Block Design`
		SwitchArg streamArg("", "stream", "Enable QRNG streaming", false);   // added to allow QRNG data streaming`
		ValueArg<long> Si_tstArg("s", "si_test",								// added this to allow user to select Si_test w/ opt len
					"Enable Si_tst with opt length (0 implies m)",
					false, 0, "Integer");

		ValueArg<unsigned long> nArg("n", "inputsize",
					"Length of input data (bits)",
					false, 0, "Integer");
		ValueArg<unsigned long> mArg("m", "outputsize",			// changed "true" to "false" in mArg's parameters, 
					"Length of extracted data (bits)",				// this makes it NOT required to give an m to run the program
					false, 0, "Integer");
		ValueArg<string> weakdesArg("w", "weakdes",				// in the 3rd parameter deleted options block and aot, not used in this version
		...
		ValueArg<string> bitextArg("x", "bitext",
					    "Bit extractor construction (rsh)",		// in the 3rd parameter deleted options xor and lu, not used in this version
		...
		ValueArg<int> numTasksArg("", "numtasks",
					  ...
					  false, -1, "Integer");
		ValueArg<int> numThreadsArg("t", "numthreads",
					  "Number of PThreads (0 means no threading)",
					  false, 0, "Integer");
		...
		cmd.add(megaArg);
		cmd.add(verboseArg);
		cmd.add(blockArg);		// add Blk_Design
		cmd.add(streamArg);		// add QRNG data streaming
		cmd.add(dryrunArg);
		...
		if (params.pp.i != "NA"){					// if there is a user-defined source bits file, check its size vs "n"
			...
			if(n_file.is_open())									
			{
				...
				cout << endl<< endl<<"Your file size is: "<< size << " you requested " << params.pp.n << " bits" << endl;
				uint64_t size_n = (size/64) * 64;
				if (params.pp.n > size) {		// size_n = # of items in that file, divisible by 64.
					params.pp.n = size_n;				
					cout << "Insufficient bits in file, revising n=" << params.pp.n << endl; 
				} else {
					cout << "Input Data file exceeds specified input size of " << params.pp.n 
							<< ", will only use " << params.pp.n << " from file" << endl;
				}
				n_file.close();
			}
		}															//*
		...
		params.si_tst = Si_tstArg.getValue();		// add opt Si_test
		params.blk_des = blockArg.getValue();		// add opt Block design
		params.streaming = streamArg.getValue();	// add opt for QRNG data straeming

		params.pp.of = outfArg.getValue();			// added variable to store output data file name
		if (params.pp.of != "NA"){ 
			cout << "optional output extracted data file name=" << params.pp.of
                             << endl;
		}
		...
		if ( params.pp.alpha > 1.0) {  	// aassume entropy input is actual number of bits
			cout << "min-entropy, alpha=" << params.pp.alpha << endl;
			params.pp.alpha = (params.pp.alpha+1.0)/params.pp.n;
		}
		if (params.streaming==false && (params.pp.alpha <= 0 || params.pp.alpha >= 1) ) {
		...
		params.num_threads = numThreadsArg.getValue();
		if (params.num_threads > 0)
			cout << "PThreads = " << params.num_threads  << endl;
#ifndef NO_DEBUG
		...
		params.pp.m *= mult_factor;	//  moved before "if" clause so any use defined values would be augmented
						// by the "multi_factor", but any derived values fromd "params.pp.n" already incorp "multi_factor"		
...
int dispatch(struct params &params) {
	 ...
	 if (params.streaming){			// for streaming option, start server, wait for connections & init fd's
	 	...
	 	// don't allocate within IF, because we want the lists to persist outside of IF 
 	 Thread_arg thread_data[params.num_threads]; // allocate a list of thread_args	
 	 pthread_t  thread[params.num_threads];	    // allocate thread IDs
	 	// if PThreads requested, spawn the threads here before STREAM loop 
	 if (params.num_threads > 0) {
		...
		for (int i=0; i<params.num_threads; i++) {
	    		if( 0 != pthread_create(&thread[i], NULL, trevisan_extract_thread, (void*)&thread_data[i]) ) {
    		  		printf("ERROR pthread error on inst[%i]\n",i);
    		  		perror("reason");
			}
	    	}
	 }
...
NEXT_STREAM:
	 ...
	 if (params.streaming) {	// for streaming option, get cmd_line info & then weak data
		stream_get_cmd_line_bits(socket_fd_data, &params.pp.n, &params.pp.m, &params.pp.eps, &params.pp.alpha, &params.blk_des);
		...
		global_rand_vector.at(params.pp.n/64) &= part_mask(params.pp.n);   // clr out unneeded lower bits when bit len is not aligned at 64
		global_rand = &global_rand_vector;
  cout << "QRNG Streaming CMD and Weak Data Done" << endl; 
	 } else if (params.pp.i == "NA"){										// if no user-given source bits file, get bits with create_randomness(...)
		prng *random = new prng;												// create new prng object in order to use prng.hpp's PRNG
        //      vector<uint64_t> global_rand_vector; // chg Mink 7/25/2014, pass ptr not a copy of "global_rand_vector"
		global_rand = random->create_randomness(params.pp.n, &global_rand_vector); // def in "prng.hpp"
...
// printf("global_rand=%p\n", global_rand);
		...
		const int size = params.pp.n/64 + 1;	// extract the bits in chunks of 64, storing them in 64-bit unsigned long long ints in an array
		global_rand_vector.resize(size); 		
		ifstream data_file (params.pp.i);
		if(data_file.is_open()) {

			uint64_t len = rd_ascii_bit_file(data_file, params.pp.n, &global_rand_vector); // replace code with a better fct
			if ( len < params.pp.n ) {
				cout << "Insufficient input data in file, only " << len << " instead of specified " << params.pp.n << endl;
				exit(-1);
			}
			data_file.close();
		} else {
			cout << "****Can't open specified input data file " << params.pp.i << ", Terminating Program" << endl;
			exit(-1);
		}
...
cout << "m=" << params.pp.m << ", m_req=" << params.pp.m_req << endl;
		 params.pp.a = 0;
		 bitext_rsh *btx_rsh = new bitext_rsh();	// removed r_interp parameter
...
// printf("global_rand_vector.at(0)=%ld\n", global_rand_vector.at(0) );
		 btx_rsh->set_input_data(global_rand, params.pp);  // def in "1bitext_rsh.h"
		 bext = btx_rsh;
		 break;
	 }	
	 default:
